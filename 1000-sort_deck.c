...
{
	deck_node_t *back = card->prev, *current_pos = card;
	/*NULL, 19, 48, 9, 71, 13, NULL*/

	back->next = current_pos->next;
	if (current_pos->next)
		current_pos->next->prev = back;
	current_pos->next = back;
	current_pos->prev = back->prev;
	back->prev = current_pos;
	if (current_pos->prev)
		current_pos->prev->next = current_pos;
	else
		*deck = current_pos;
	return (current_pos);
}
...
void insertion_sort_deck(deck_node_t **deck)
{
	...
	while (node)
	{
		...
		if (node->prev)
		{
			...
			value_current1 = get_card_position(node);
		}
		while ((node->prev) && (value_prev > value_current1))
		{
			value_prev = get_card_position((node->prev));
			value_current1 = get_card_position(node);
